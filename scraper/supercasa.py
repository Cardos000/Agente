...
from config import MAX_PRICE, MIN_AREA

class SuperCasaScraper(BaseScraper):
    """
    Scraper para SuperCasa.pt (imobiliário)
    ...
    def run(self, page) -> List[RawListing]:
        """Scrape SuperCasa com autenticação e anti-bot"""
        all_listings = []
        try:
            self._handle_authentication(page)
            url = self._build_search_url()
            self.logger.info("Iniciando scrape SuperCasa")
            listings = self._scrape_with_pagination(page, url)
            all_listings.extend(listings)
            self.stats["listings_found"] = len(all_listings)
        except Exception as e:
            self.logger.error(f"Erro fatal no SuperCasa: {e}")
            self.stats["errors"] += 1
        return all_listings

    def _build_search_url(self) -> str:
        """Construir URL de busca"""
        # SuperCasa: apartamentos, Porto, T2, até MAX_PRICE
        return (
            f"{self.BASE_URL}/comprar-apartamentos/t2/porto"
            f"?ordem=recente&preco-max={MAX_PRICE}&area-min={MIN_AREA}"
        )
    ...
    def _dismiss_modals(self, page):
        ...
        try:
            page.click('button[class*="cookie"]', timeout=2000)
        except:
            pass
    ...
    def _extract_listings(self, page) -> List[RawListing]:
        ...
        try:
            # SuperCasa usa divs com classe específica
            items = page.locator('.property-item').all()
            
            if not items:
                items = page.locator('[data-property-id]').all()
